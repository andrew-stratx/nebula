...
function getShortenedSpinContext(spinContext) {
    return crypto.createHash('sha256').update(spinContext).digest('hex').substr(0, 6);
}

function getBoyarBucketName(spinContext) {
    return `boyar-${getShortenedSpinContext(spinContext)}`;
}
...
class TerraformService {
    ...
    createTerraformVariablesFile({ spinContext, cloud, keys }) {
        const shortenedSpinContext = getShortenedSpinContext(spinContext);
        let contentAsString = '';
        ...
        if (cloud.type === types.clouds.aws) {
            ...
            contentAsString += `aws_orbs_worker_instance_count = "${cloud.nodeCount || 2}"\n`;
            ...
            const boyarBucket = getBoyarBucketName(spinContext);
